...
class BrowseBrickController extends BrickController
{
	...
	{
        /** @var ScopeValidatorHelper $scopeValidator */
	    $scopeValidator = $this->get('scope_validator');

	    /** @var boolean $isDebugEnabled */
        $isDebugEnabled = $this->getParameter('kernel.debug');

        /** @var Router $router */
	    $router = $this->get('router');

	    /** @var BricksCollection  $bricksCollection */
        $bricksCollection = $this->get('bricks_collection');

        /** @var \Combodo\iTop\Portal\Brick\BrowseBrick $oBrick */
        $oBrick = $bricksCollection->getBrickById($sBrickId);
		...
		$sDataLoading = ($sDataLoading !== null) ? $sDataLoading : $this->get('request_manipulator')->ReadParam('sDataLoading', $oBrick->GetDataLoading());
		// Getting search value
		$sSearchValue = $this->get('request_manipulator')->ReadParam('sSearchValue', '');
		...
		{
			return new Response(
                'Browse brick "' . $sBrickId . '" : Unknown browse mode "' . $sBrowseMode . '", availables are ' . implode(' / ', array_keys($aBrowseModes)),
                500
            );
		}
		...
		{
            return new Response(
                'Browse brick "' . $sBrickId . '" : No levels to display.',
                500
            );
		}
		...
		if ($sDataLoading === AbstractBrick::ENUM_DATA_LOADING_LAZY)
		{
			switch ($sBrowseMode)
			{
				case BrowseBrick::ENUM_BROWSE_MODE_LIST:
					...
					$iPageNumber = (int) $this->get('request_manipulator')->ReadParam('iPageNumber', 1, FILTER_SANITIZE_NUMBER_INT);
					$iListLength = (int) $this->get('request_manipulator')->ReadParam('iListLength', BrowseBrick::DEFAULT_LIST_LENGTH, FILTER_SANITIZE_NUMBER_INT);
				...
				case BrowseBrick::ENUM_BROWSE_MODE_MOSAIC:
					// Retrieving parameters
					$sLevelAlias = $this->get('request_manipulator')->ReadParam('sLevelAlias', '');
					$sNodeId = $this->get('request_manipulator')->ReadParam('sNodeId', '');
		...
		while ($aCurrentRow = $oSet->FetchAssoc())
		{
			switch ($sBrowseMode)
			{
				case BrowseBrick::ENUM_BROWSE_MODE_TREE:
				case BrowseBrick::ENUM_BROWSE_MODE_MOSAIC:
					static::AddToTreeItems($router, $aItems, $aCurrentRow, $aLevelsProperties, null);
					break;
	...
	public static function TreeToFlatLevelsProperties(array $aLevels, array &$aLevelsProperties, $sLevelAliasPrefix = 'L', BricksCollection $bricksCollection, ScopeValidatorHelper $scopeValidator, Router $router, $isDebugEnabled)
	{
        ...
        foreach ($aLevels as $aLevel)
		{
			...
			$oScopeSearch = $scopeValidator->GetScopeFilterForProfiles(UserRights::ListProfiles(), $oSearch->GetClass(), UR_ACTION_READ);
					...
					if (!array_key_exists($sId, $aLevelsProperties[$sCurrentLevelAlias]['actions']))
					{
							...
							switch ($aAction['type'])
							{
								case BrowseBrick::ENUM_ACTION_CREATE_FROM_THIS:
									...
									if ($aAction['factory']['type'] === BrowseBrick::ENUM_FACTORY_TYPE_CLASS)
									{
										...
										$aAction['url'] = $router->generate('p_object_create', array('sObjectClass' => $aAction['factory']['value']));
									}
									else
									{
						...
						switch ($aAction['type'])
						{
							case BrowseBrick::ENUM_ACTION_CREATE_FROM_THIS:
								...
								{
									$aAction['url'] = $router->generate('p_object_create_from_factory', array('sEncodedMethodName' => base64_encode($aAction['factory']['value']), 'sObjectClass' => '-objectClass-', 'sObjectId' => '-objectId-'));
								}
								break;
						}
	 ...
	 * @throws \Exception
	 */
	public static function AddToFlatItems(array $aCurrentRow, array &$aLevelsProperties, Router $router)
	{
		$aRow = array();
                ...
                if ($aLevelsProperties[$key][$sOptionalAttribute] !== null)
                {
	                ...
	                if($oAttDef instanceof AttributeImage)
	                {
		                ...
		                if ($sOptionalAttribute === 'image_att')
		                {
			                ...
			                {
				                $tmpAttValue = $router->generate('p_object_document_display', array(
	 ...
	 * @throws \Exception
	 */
	public static function AddToTreeItems(Router $router, array &$aItems, array $aCurrentRow, array &$aLevelsProperties, $aCurrentRowObjects = null)
	{
                ...
                if ($aLevelsProperties[$aCurrentRowKeys[0]][$sOptionalAttribute] !== null)
                {
                    ...
                    if($oAttDef instanceof AttributeImage)
                    {
	                    ...
	                    if($sOptionalAttribute === 'image_att')
	                    {
	                        if (is_object($tmpAttValue) && !$tmpAttValue->IsEmpty())
	                        {
	                            $tmpAttValue = $router->generate('p_object_document_display', array('sObjectClass' => get_class($aCurrentRowValues[0]), 'sObjectId' => $aCurrentRowValues[0]->GetKey(), 'sObjectField' => $aLevelsProperties[$aCurrentRowKeys[0]][$sOptionalAttribute], 'cache' => 86400));
	                        }
	                        else
	                        {
	                            $tmpAttValue = $oAttDef->Get('default_image');
		...
		{
			static::AddToTreeItems($router, $aItems[$sCurrentIndex]['subitems'], $aCurrentRowSliced, $aLevelsProperties, $aCurrentRowObjects);
		}
	}
